              ...
              <div className='flex justify-between flex-col gap-1 font-bold'>
                ...
                <span className='text-lg text-[#3a618c]'>
                  {isLoading ? (
                    ...
                    `₩${Math.floor((orderDetail?.totalPayment ?? 0)).toLocaleString()}`
                  )}
                </span>
              </div>
            </div>
                ...
                <div key={index} className='flex justify-between flex-col gap-1'>
                  ...
                  <span className='text-sm font-medium ml-0.5'>
                    {isLoading ? (
                      ...
                      (orderDetail?.[
                        ['orderId', 'createdDate', 'productName', 'totalQuantity'][index]
                      ] ?? '정보 없음')
                    )}
                  </span>
                </div>
                ...
                <div key={index} className='flex justify-between flex-col gap-1'>
                  ...
                  <span className='text-sm font-medium ml-0.5'>
                    {isLoading ? (
                      ...
                      (orderDetail?.[['username', 'phoneNumber', 'address'][index]] ?? '정보 없음')
                    )}
                  </span>
                </div>
              ))}
