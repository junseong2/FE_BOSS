...
function PaymentPage() {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // ✅ 1. 주문 생성 (Order)
      const orderResponse = await fetch("http://localhost:5000/orders/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId, totalPrice: orderTotal }),
      });
      ...
      IMP.request_pay(
        ...
        async (rsp) => {
          if (rsp.success) {
            ...
            const paymentResponse = await fetch("http://localhost:5000/payment/portone", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            // ✅ 4. 결제 상태 업데이트 (PAID)
            await fetch("http://localhost:5000/payment/update-status", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ impUid: rsp.imp_uid, status: "PAID" }),
            });
            ...
            // ✅ 5. 결제 실패 시 상태 업데이트 (FAILED)
            await fetch("http://localhost:5000/payment/update-status", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ impUid: rsp.imp_uid, status: "FAILED" }),
            });
          }
        }
      );
        ...
        <div className="mt-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {["totalpay", "kakaopay", "tosspay", "paycopay"].map((method) => (
              <button
                key={method}
                className={`p-3 text-center rounded-lg font-semibold border transition ${selectedPaymentMethod === method
