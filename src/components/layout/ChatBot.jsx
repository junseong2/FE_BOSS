...
function ChatBot() {
  ...
  return (
    <div>
      <div
        className='fixed bottom-4 right-4 w-16 h-16 z-[40] cursor-pointer shadow-xl rounded-full bg-white border border-gray-200 flex items-center justify-center hover:scale-105'
        onClick={toggleChat}
      >
        <img src={bossLogo} alt='Chat Icon' className='w-24 h-20' />
      </div>

      {/* 챗봇 */}
      <div
        className={`${isOpen ? 'visible opacity-90' : 'invisible opacity-0'} backdrop-blur-lg transition-all fixed bottom-24 right-4 w-96 max-w-[95vw] h-[550px] bg-white shadow-2xl border border-gray-200 rounded-3xl flex flex-col z-[9999] overflow-hidden animate-fadeInUp`}
      >
          ...
          <div className='relative z-10'>
            {messages.map((msg, index) => (
              <div
                key={index}
                ref={divRef}
                className={`rounded-xl px-3 py-2 my-4 text-sm max-w-[80%] break-words transition-all opacity-90 ${
                  msg.sender === 'user'
...
function ProductRecommendationCard({ productId, reason }) {
  ...
  return (
    <div className='mt-2 border border-gray-200 rounded-xl overflow-hidden'>
      ...
      >
        <img src={product.gImage[0]} alt={product.name} className='w-full h-28 object-cover' onError={(e)=>{
